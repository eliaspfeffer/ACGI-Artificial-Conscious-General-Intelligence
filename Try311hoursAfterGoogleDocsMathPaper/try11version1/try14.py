...
ram_memory = {
    "K1": ["Ein Apfel hat eine grüne Farbe"],
    "K2": ["Ein Baum hat grüne Blätter"],
    "K3": ["Chlorophyll verursacht eine grüne Farbe"],
...
def plot_knowledge_graph_3d():
    ...
    for context, infos in ram_memory.items():
        G.add_node(context, color='blue')
        for info in infos:
            G.add_node(info, color='green')
            G.add_edge(context, info)  # Connect context to its information
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title("AGI Kontextbewegung in 3D mit Informationen")
    plt.show()
