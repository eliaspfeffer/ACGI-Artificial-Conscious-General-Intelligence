...
store_in_ram("K1", "Der Apfel ist grün")
...
def plot_knowledge_graph():
    ...
    # Knoten aus Kontexten und deren Verbindungen
    for context, words in ssd_memory["relations"].items():
        G.add_node(context, color='red')
        for word in words:
            if word in ssd_memory["words"]:
                G.add_edge(context, word)
    
    # Zeichnen des Graphen
    plt.figure(figsize=(8, 6))
    colors = [G.nodes[n]['color'] for n in G.nodes]
    nx.draw(G, with_labels=True, node_color=colors, edge_color='gray', font_size=10, node_size=1000)
    plt.title("Wissensgraph der AGI")
    plt.show()
