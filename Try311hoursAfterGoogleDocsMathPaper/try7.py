...
consistencies = {
    ("K1", "K3"): jaccard_index(extract_attributes_from_sentence("Der Apfel ist grün"), extract_attributes_from_sentence("Chlorophyll verursacht grüne Farbe")),
    ("K2", "K4"): jaccard_index(extract_attributes_from_sentence("Ein Baum hat grüne Blätter"), extract_attributes_from_sentence("Ein Baum ist im Apfel")),
    ("K1", "K2"): jaccard_index(extract_attributes_from_sentence("Der Apfel ist grün"), extract_attributes_from_sentence("Ein Baum hat grüne Blätter")),
}
...
def plot_knowledge_graph():
    ...
    colors = ['red' if n in truth_values else 'green' for n in G.nodes]
