...
class Information:
    ...
    def __repr__(self):
        ...
        # Verkürze Content auf ~30 Zeichen
        short_content = (self.content[:30] + "...") if len(self.content) > 30 else self.content
        return f"Information[{cat}]<{short_content}>"
...
class ThoughtEngine:
    ...
    def choose_next_info(self) -> Information:
        ...
        if not neighbors:
            # Keine Nachbarn => bleib bei der aktuellen Info
            return self.current_info
    ...
    def step(self) -> Information:
        ...
        if random.random() < self.new_thought_probability:
            ...
            print(f"Neuer Gedanke entwickelt: \"{new_content}\" "
                  f"=> verknüpft mit: \"{next_info.content[:50]}\"")
        
        # ** DEBUG-LOG am Ende: **
        # Zeige den aktuellen "Fokus" und Anzahl Links
        neighbor_count = len(next_info.links)
        print(f" [Debug] Denke jetzt über: \"{next_info.content[:50]}\" "
              f"(Links: {neighbor_count}, StepsInFocus={self.steps_in_focus})")
    ...
    def run_infinite_loop(self):
        ...
        try:
            while True:
                ...
                # Logge alle 5 Schritte zusätzlich eine Übersicht
                if step_counter % 5 == 0:
